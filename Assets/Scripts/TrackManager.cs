...
public class TrackManager : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        for (int j = 0; j < tracksObject.transform.childCount; j++)
        {
            Vector3 trackPointPos = tracksObject.transform.position;
            if (j != 0)
                points.Add(new TrackPoint(Vector3.Distance(points[j - 1].position, tracksObject.transform.GetChild(j).position), tracksObject.transform.GetChild(j).position));
            else
                points.Add(new TrackPoint(0, tracksObject.transform.GetChild(j).position));
        }
