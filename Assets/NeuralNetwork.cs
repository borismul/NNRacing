...
public class NeuralNetwork
{
    ...
    public List<float> GetOutput(List<float> input)
    {
        ...
        {
            Debug.LogError("Number of inputs most be equal to inputs");
            return null;
        }
    ...
    public void VisualizeNetwork(GameObject perceptron, GameObject hSpace, GameObject vSpace, GameObject line, string name)
    {
        ...
        GameObject connections = (GameObject)GameObject.Instantiate(temp, canvas.transform);
        connections.name = "Connections";
        connections.AddComponent<RectTransform>().anchorMax = new Vector2(1, 1);
        connections.GetComponent<RectTransform>().anchorMin = new Vector2(1, 1);
        ...
        GameObject perceptrons = (GameObject)GameObject.Instantiate(temp, canvas.transform);
        perceptrons.name = "Perceptrons";
        perceptrons.AddComponent<RectTransform>().anchorMax = new Vector2(1, 1);
        perceptrons.GetComponent<RectTransform>().anchorMin = new Vector2(1, 1);
        ...
        for (int i = 0; i < layers.Count; i++)
        {
            ...
            for (int j = 0; j < layers[i].Count; j++)
            {
                ...
                Vector2 curPos = -startPos + new Vector2(-(layers.Count - 1) * (hSpaceSize.x + perceptronSize.x) + (layers[i][j].layer * (hSpaceSize.x + perceptronSize.x)), -layers[i][j].number * (perceptronSize.y + vSpaceSize.y));
                GameObject CurPerceptron = (GameObject)GameObject.Instantiate(perceptron, perceptrons.transform);
                ...
                foreach (PerceptronConnection connection in layers[i][j].inputConnections)
                {
                    ...
                    Vector2 inPos = -startPos + new Vector2(-(layers.Count - 1) * (hSpaceSize.x + perceptronSize.x) + (inPerceptron.layer * (hSpaceSize.x + perceptronSize.x)), -inPerceptron.number * (perceptronSize.y + vSpaceSize.y)) + Vector2.right * perceptronSize.x / 2;
                    Vector2 outPos = curPos + Vector2.left * perceptronSize.x / 2;
                    float rotation = Mathf.Atan2(inPos.y - outPos.y, inPos.x - outPos.x);
                    GameObject curConnection = (GameObject)GameObject.Instantiate(line, connections.transform);
